...
| UI | [uiweft](https://github.com/zouh9426/uiweft) orchestration | `DESIGN.md` (generated by uiweft) | Projects with a UI only |
...
- **Info collected once, up front**: six groups (identity, GitHub, server, domain, app secrets, backup policy) are asked once and land in `PROJECT_INDEX.md` and the deployment doc; any group may be deferred ("TBD"), which creates tracked TODOs instead of blocking. **Secrets themselves never enter the repo** — only locations and references.
...
2. Optional: [uiweft](https://github.com/zouh9426/uiweft) (UI track; everything else works without it)
...
The optional dependency uiweft is licensed by its own author — check its upstream repository before installing.
