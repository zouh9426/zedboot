...
| Deployment | Dedicated account + tiered Docker permissions + rsync push + private Git backup (standard-tier account isolation is not a host security boundary) | Dockerfile & friends + `docs/guides/deployment.md` | Deployable code projects only |
...
- **Info collected once, up front**: six groups (identity, GitHub, server, domain, app secrets, backup policy) are asked once; any group may be deferred ("TBD"), which creates tracked TODOs instead of blocking. **Privacy line**: derivable values (account = project name is only a suggested default, independently configurable — `docs/private/deploy.env` is authoritative; `/opt/<project>` paths) land in `PROJECT_INDEX.md` and the deployment doc; non-derivable ops values (IP / SSH port / key paths / crontab schedule) live only in local `docs/private/ops.md` (gitignored, never committed) while committed docs carry placeholders — a project can go public anytime without leaking its infrastructure fingerprint; the six facts the deploy scripts consume additionally live in `docs/private/deploy.env` (also gitignored, never committed). **Secrets themselves never enter the repo — nor the chat context**: only key names are asked, never values; users write values into `.env` themselves, and only locations and references are recorded.
...
tests/                        ← audit.py + verify.py + pre-push hook + deploy template tests (unittest, runs in CI)
