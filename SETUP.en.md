...
Search the skills directory recursively for `SKILL.md` files and read the `name:` field in their frontmatter to check whether these skills are installed (**match by name, not directory name** — a directory name may differ from the skill name; name matching is case-insensitive. If you use `find`, pass `-L` to follow symlinks, otherwise symlink-installed skills are missed):
